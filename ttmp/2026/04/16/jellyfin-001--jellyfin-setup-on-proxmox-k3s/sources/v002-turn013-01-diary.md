...
| Terraform | Yes - `/home/manuel/code/wesen/corporate-headquarters/go-go-infra/proxmox/` |
| Deployment Options | k3s deployment, new VM (via Terraform), or LXC |
| Goal | Jellyfin media server |

### Terraform Setup Notes

The Terraform infrastructure at `corporate-headquarters/go-go-infra/proxmox/` provisions VMs and uses Ansible for configuration. This pattern should be followed for Jellyfin deployment if using a separate VM.
